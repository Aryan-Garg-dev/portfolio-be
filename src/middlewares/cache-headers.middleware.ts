...
type TCacheOptions<T extends TCacheBehavior = TCacheBehavior> = {
	...
	: T extends "fresh" | "stale-ok"
		? { maxAge?: number } & (T extends "stale-ok" ? { staleWhileRevalidate?: number } : {})
...
const BEHAVIOR_CONFIGS = {
	static: { maxAge: 31536000, directives: ["immutable"] },
	fresh: { maxAge: 300, directives: ["must-revalidate"] },
	"stale-ok": { maxAge: 600, staleWhileRevalidate: 1200, directives: [] },
	"no-cache": { maxAge: 0, directives: ["no-cache", "must-revalidate"] },
	"no-store": { maxAge: 0, directives: ["no-store", "no-cache", "must-revalidate"] },
} as const;
...
function buildCacheConfig(options: TCacheOptions): ICacheConfig {
	...
	if (options.behavior === "stale-ok") {
		const swr =
			"staleWhileRevalidate" in options
				? options.staleWhileRevalidate
				: "staleWhileRevalidate" in behaviorConfig && behaviorConfig.staleWhileRevalidate;
		if (swr && swr > 0) {
